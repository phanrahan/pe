...
# Here we model the pe as a pure (stateless) closure. For this reason, we ignore
# stateful components such as reigsters, memory, etc. The function
# pe_generator() returns a 5-argument function which models a "pre-configured"
# pe, i.e. it can be called just using dynamic/runtime arguments (data0, data1,
# etc.). In this way, the returned pe is a tile of a spatial architecture
# without dynamic instruction fetch. In order to use different configurations of
# the pe, we must "re-configure" the pe, i.e. call pe_generator() again.
def generate_pe(lut_code, data0_const, data1_const, bit0_const, bit1_const, bit2_const, debug_trig, debug_trig_p, bit2_mode, bit1_mode, bit0_mode, data1_mode, data0_mode, flag_sel, irq_en, acc_en, signed, operation):
    def pe_fn(data0_in, data1_in, bit0_in, bit1_in, bit2_in):
    ...
    return lambda data0, data1, bit0, bit1, bit2: pe_fn(data0, data1, bit0, bit1, bit2)
